...
class Twibber {
	...
	$text = preg_replace('/\s?@(.+)\s?/','<a href="?nick=$1" onclick="return dyn_get(true, false, this.innerText.replace(/@/,\'\'));">@$1</a> ',$text);
	$text = preg_replace('/\s?#(.+)\s?/',' <a href="?search=$1" class="hash" onclick="return dyn_get(true, false, false, this.innerText.replace(~#~,\'\'));">#$1</a> ',$text);
	return $text;
    }
}
class wcf{
